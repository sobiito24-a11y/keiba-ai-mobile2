...
def attach_nar_pure_top5_columns(
    ...
    *,
    ...
    fields = (
        ...
        "partner_trust_score",
        "partner_trust_level",
        "partner_trust_reason",
        "race_purchase_judgement",
        "race_purchase_label",
        "race_purchase_score",
        "race_purchase_reason",
        "ability_gap_1_2",
        "ability_gap_1_2_level",
        "honmei_market_rank",
        "honmei_odds",
        "win_bet_allowed",
        "win_bet_block_reason",
        "trusted_partner_count",
        "recommended_ticket_mode",
        "top5_data_shortage_count",
...
def render_market_race_facts(result: PredictionResult, table: pd.DataFrame) -> None:
    ...
    if clean_text(result.race_mode).lower() == "jra":
        ...
        summary = nar_top5_summary_lines(comparison)
    st.markdown(
...
def render_full_field_comparison(
    ...
    *,
    ...
    if is_jra:
        ...
        st.markdown(full_field_nar_top5_comparison_html(comparison), unsafe_allow_html=True)
        st.caption("NAR Top5順位を基準に、保存済みの全頭材料を比較しています。")
    with st.expander("研究・監査情報", expanded=False):
...
def nar_top5_summary_lines(comparison: dict[str, Any]) -> list[str]:
    rows = [row for row in comparison.get("rows") or [] if isinstance(row, dict)]
    if not rows:
    ...
    ability_sorted = sorted(
        rows,
        key=lambda row: (
            -(to_float(row.get("nar_pure_ability_score")) if to_float(row.get("nar_pure_ability_score")) is not None else -9999.0),
            int(to_float(row.get("number")) or 999),
        ),
    )
    top_ability = ability_sorted[0] if ability_sorted else {}
    ...
    gap = None
    if top_ability and second_ability:
        first = to_float(top_ability.get("nar_pure_ability_score"))
        second = to_float(second_ability.get("nar_pure_ability_score"))
        if first is not None and second is not None:
            gap = first - second
    leader = top5[0] if top5 else {}
    ...
    return [
        ...
        + horse_label_for_summary(top_ability)
        + f"、◎○能力差{gap_text}",
        "Top5首位：" + horse_label_for_summary(leader),
        f"Top5内の4角前方：{len(front_top5)}頭、注意馬：{len(warnings)}頭",
    ]
...
def nar_warning_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        if truthy_display(row.get("nar_warning_candidate")):
            out.append(row)
    return sorted(out, key=nar_top5_row_sort_key)


def nar_purchase_judgement_html(comparison: dict[str, Any]) -> str:
...
def nar_top5_conclusion_html(comparison: dict[str, Any]) -> str:
    rows = [row for row in comparison.get("rows") or [] if isinstance(row, dict)]
    recommendations = comparison.get("nar_top5_recommendations")
    if not isinstance(recommendations, list) or not recommendations:
        recommendations = [row for row in sorted(rows, key=nar_top5_row_sort_key) if (to_float(row.get("nar_top5_rank")) or 999) <= 5]
    summary_lines = nar_top5_summary_lines(comparison)
    summary = (
        '<div class="ka-dashboard-card">'
        '<div class="ka-dashboard-title">NAR Top5サマリー</div>'
        '<div class="ka-note">'
        + "<br>".join(plain_text_to_html(line) for line in summary_lines)
        + "</div></div>"
    )
    ...
    for horse in sorted(recommendations, key=nar_top5_row_sort_key)[:5]:
        ...
        rank = to_float(horse.get("nar_top5_rank"))
        pure_line = f"純能力 {number_display(horse.get('nar_pure_ability_score'))}（{rank_display(rank)}）"
        if rank == 2 and to_float(gap_1_2) is not None and to_float(gap_1_2) <= 1.0:
            pure_line += " / 同格圏"
        ...
        if condition_materials:
            detail_lines.append(f"条件材料：{condition_materials}")
    ...
    if warnings:
        blocks = []
        for horse in warnings[:3]:
            title = join_nonempty(["✓", clean_text(horse.get("number")), clean_text(horse.get("name"))], sep=" ")
            reason = nar_short_attention_reason(horse)
            detail_lines = [
            ...
            blocks.append(
                ...
                f'<b>{plain_text_to_html(title)}</b>'
                f'<div class="ka-note">{plain_text_to_html(" / ".join(detail_lines))}</div>'
                f'<div class="ka-note">注目理由：{plain_text_to_html(reason)}</div>'
                '</div>'
            )
        warning_html = (
            '<div class="ka-dashboard-card">'
            '<div class="ka-dashboard-title">✓注目馬</div>'
            '<div class="ka-recommend-grid">'
            + "".join(blocks)
...
def full_field_nar_top5_comparison_html(comparison: dict[str, Any]) -> str:
    ...
    for row in source_rows:
        if not isinstance(row, dict):
            continue
        normalized = dict(row)
        normalized["nar_display_mark"] = nar_display_mark_from_row(normalized)
    ...
    metrics: list[tuple[str, str, Any]] = [
        ...
        ("NAR最終印", "", lambda horse: nar_display_mark_from_row(horse) or "—"),
        ...
        ("相手信頼度", "", lambda horse: clean_text(horse.get("partner_trust_level")) or "—"),
        ("総合注目度", "", lambda horse: nar_total_attention_rank(horse)),
        ("✓注目馬", "", lambda horse: "✓" if truthy_display(horse.get("nar_warning_display")) else "—"),
        ("注目理由", "", lambda horse: nar_short_attention_reason(horse) if truthy_display(horse.get("nar_warning_display")) else "—"),
        ("単勝オッズ", "", lambda horse: format_odds(pick(horse, "actual_odds", "単勝オッズ", "オッズ", "単勝")) or "—"),
        ("騎手", "", comparison_jockey_info_text),
        ("脚質", "", lambda horse: clean_text(horse.get("running_style")) or "—"),
        ("4角位置", "position", lambda horse: clean_text(horse.get("corner4_display")) or comparison_position_icon(clean_text(horse.get("corner4_group")))),
        ("近3走指数", "", lambda horse: clean_text(horse.get("recent3_indices")) or "—"),
        ("距離指数", "", lambda horse: clean_text(horse.get("distance_index")) or "—"),
    ...
    return comparison_table_html(rows, metrics, mark_key="nar_display_mark")
...
def render_v2_overview(comparison: dict[str, Any]) -> None:
    summary = comparison.get("v2_summary") if isinstance(comparison.get("v2_summary"), dict) else {}
    recommendations = comparison.get("v2_recommendations")
    if summary:
        st.markdown(v2_summary_html(summary), unsafe_allow_html=True)
    if isinstance(recommendations, list) and recommendations:
        st.markdown(v2_recommendations_html(recommendations), unsafe_allow_html=True)
    st.markdown(
        '<div class="ka-dashboard-card">'
        '<div class="ka-dashboard-title">評価の見方</div>'
        '<div class="ka-note">'
        'AI点：今回の総合評価 / 能力：基礎能力 / 条件：コース・距離実績 / '
        '展開：今回ペース×脚質 / 近走・状態：直近状態 / 騎手：名前＋複勝率。'
        '</div></div>',
        unsafe_allow_html=True,
    )


def v2_summary_html(summary: dict[str, Any]) -> str:
    cards = [
        ("能力", clean_text(summary.get("能力")) or "能力値を土台に比較"),
        ("再現性", clean_text(summary.get("再現性")) or "今回条件の実績を確認"),
        ("展開", clean_text(summary.get("展開")) or "想定ペースと脚質"),
        ("近走/状態", clean_text(summary.get("近走/状態")) or "直近状態の加減点"),
        ("今回評価", clean_text(summary.get("今回評価")) or "AI点の上位馬"),
    ]
    inner = "".join(
    ...
    return (
        '<div class="ka-dashboard-card">'
        '<div class="ka-dashboard-title">v2 AI評価サマリー</div>'
        f'<div class="ka-summary-grid">{inner}</div>'
        '<div class="ka-note">能力 → 条件 → 展開 → 近走/状態 → AI点 → 印。通常画面の結論はv2 AI点に一本化しています。</div>'
        '</div>'
    )


def v2_recommendations_html(recommendations: list[dict[str, Any]]) -> str:
    cards: list[str] = []
    for horse in recommendations[:5]:
        title = " ".join(
            part
            for part in [
                clean_text(horse.get("v2_final_mark")),
                clean_text(horse.get("number")),
                clean_text(horse.get("name")),
                f"AI点 {number_display(horse.get('v2_ai_score'))}",
            ]
            if part
        )
        components = " ｜ ".join(
            part
            for part in [
                f"能力 {number_display(horse.get('v2_ability_score'))}",
                f"条件 {signed_number_display(horse.get('v2_condition_score'))}",
                f"展開 {signed_number_display(horse.get('v2_pace_score'))}",
                f"近走/状態 {signed_number_display(horse.get('v2_recent_state_score'))}",
            ]
            if part
        )
        reason = clean_text(horse.get("v2_final_reason"))
        cards.append(
            '<div class="ka-recommend-card">'
            f'<b>{plain_text_to_html(title or "—")}</b>'
            f'<div>{plain_text_to_html(clean_text(horse.get("v2_final_role")) or "相手候補")}</div>'
            f'<div class="ka-note">{plain_text_to_html(components)}</div>'
            f'<div class="ka-note">{plain_text_to_html(comparison_jockey_brief_text(horse))}</div>'
            + (f'<div class="ka-note">根拠：{plain_text_to_html(reason)}</div>' if reason else "")
            + '</div>'
        )
    return (
        '<div class="ka-dashboard-card">'
        '<div class="ka-dashboard-title">今回の結論</div>'
        '<div class="ka-recommend-grid">'
        + "".join(cards)
        + '</div><div class="ka-note">v2 AI点順です。Baseline v0やv1の保存/派生値は監査情報に分離しています。</div></div>'
    )
...
def full_field_comparison_html(comparison: dict[str, Any], *, include_body_weight: bool = True) -> str:
    ...
    metrics: list[tuple[str, str, Any]] = [
        ...
        ("プラス材料", "plus", lambda horse: horse.get("positive_tags") or []),
        ("不安材料", "minus", lambda horse: horse.get("negative_tags") or []),
        ("v1今回評価順位", "", lambda horse: rank_display(horse.get("v1_final_rank"))),
        ("v1最終印", "", lambda horse: clean_text(horse.get("v1_final_mark")) or "—"),
        ("v1役割", "", lambda horse: clean_text(horse.get("v1_final_role")) or "—"),
        ("v1最終理由", "", lambda horse: clean_text(horse.get("v1_final_reason")) or "—"),
        ("Baseline総合順位", "", lambda horse: rank_display(horse.get("baseline_current_evaluation_rank") if horse.get("baseline_current_evaluation_rank") is not None else horse.get("current_evaluation_rank"))),
        ("Baseline印", "", lambda horse: clean_text(horse.get("baseline_mark")) or clean_text(horse.get("mark")) or "—"),
    ]
    ...
    if race_mode == "jra":
        ...
        metrics.insert(course_index + 1, ("同回り", "", lambda horse: clean_text(horse.get("same_turn_display")) or "—"))
        state_index = next((index for index, item in enumerate(metrics) if item[0] == "状態理由"), 8)
        metrics.insert(state_index + 1, ("調教評価", "", lambda horse: clean_text(horse.get("training")) or "—"))
        metrics.insert(state_index + 2, ("厩舎コメント", "", lambda horse: clean_text(horse.get("stable_comment")) or "—"))
    ...
    for horse in rows:
        ...
        mark = clean_text(horse.get("v1_final_mark")) or clean_text(horse.get("v1_mark"))
        header.append(
            "<th>"
...
def full_field_v2_comparison_html(comparison: dict[str, Any]) -> str:
    rows = comparison.get("rows") or []
    if not rows:
        return '<div class="ka-dashboard-card">比較できる出走馬データがありません。</div>'
    metrics: list[tuple[str, str, Any]] = [
        ("印", "", lambda horse: clean_text(horse.get("v2_final_mark")) or "—"),
        ("AI点", "", lambda horse: number_display(horse.get("v2_ai_score"))),
        ("トップ差", "", lambda horse: clean_text(horse.get("v2_top_gap_text")) or "—"),
        ("能力", "", lambda horse: number_display(horse.get("v2_ability_score"))),
        ("条件", "", lambda horse: signed_number_display(horse.get("v2_condition_score"))),
        ("展開", "", lambda horse: signed_number_display(horse.get("v2_pace_score"))),
        ("近走/状態", "", lambda horse: signed_number_display(horse.get("v2_recent_state_score"))),
        ("騎手", "", comparison_jockey_name_text),
        ("複勝率", "", lambda horse: clean_text(horse.get("jockey_course_place_rate")) or "—"),
        ("根拠", "", lambda horse: clean_text(horse.get("v2_final_reason")) or "—"),
    ]
    return comparison_table_html(rows, metrics, mark_key="v2_final_mark")
...
def comparison_table_html(
    ...
    *,
    ...
    for horse in rows:
        ...
        mark = clean_text(horse.get(mark_key))
        header.append(
            "<th>"
            f"{plain_text_to_html(title)}"
            + (f'<br><span class="ka-muted">{plain_text_to_html(mark)}</span>' if mark else "")
...
def comparison_jockey_name_text(horse: dict[str, Any]) -> str:
    value = clean_text(horse.get("jockey_info"))
    if value and value != "—":
        return value.split("｜", 1)[0]
    value = clean_text(horse.get("jockey_display"))
    if value and value != "—":
        return re.sub(r"\s*\d+(?:\.\d+)?%.*$", "", value).strip()
    return "—"


def comparison_jockey_brief_text(horse: dict[str, Any]) -> str:
    name = comparison_jockey_name_text(horse)
    rate = clean_text(horse.get("jockey_course_place_rate")) or "—"
    return f"{name}｜複勝率{rate}" if name != "—" else f"騎手｜複勝率{rate}"


def signed_number_display(value: Any) -> str:
    number = to_float(value)
    if number is None:
        return "—"
...
def market_horse_cards_ordered(
    ...
    *,
    ...
    if mode in {"jra", "nar"}:
        rank_column = (
            "v1_final_rank"
            if mode == "jra" and "v1_final_rank" in ordered.columns
            else "jra_top5_rank"
        ...
        ability_column = (
            ...
            if mode == "jra" and "jra_pure_ability_score" in ordered.columns
            else "nar_pure_ability_score"
            if mode == "nar" and "nar_pure_ability_score" in ordered.columns
            else "market_ability_score"
            if "market_ability_score" in ordered.columns
            else "ability_value"
            if "ability_value" in ordered.columns
            else "saved_ability_value"
            if "saved_ability_value" in ordered.columns
            else ""
        )
        ordered["_card_final_rank_sort"] = pd.to_numeric(
            ordered[rank_column] if rank_column else pd.Series(pd.NA, index=ordered.index),
            errors="coerce",
        )
        ordered["_card_top5_score_sort"] = pd.to_numeric(
            ordered[score_column] if score_column else pd.Series(pd.NA, index=ordered.index),
            errors="coerce",
        )
        ordered["_card_ability_sort"] = pd.to_numeric(
            ordered[ability_column] if ability_column else pd.Series(pd.NA, index=ordered.index),
            errors="coerce",
        )
...
def market_horse_card_html(row: dict[str, Any], race_mode: str) -> str:
    ...
    if is_jra:
        ...
        mark = nar_display_mark_from_row(row)
    else:
    ...
    detail_ability_value = (
        ...
        if is_jra
        else pick(row, "nar_pure_ability_score", "market_ability_score", "ability_value", "saved_ability_value", "能力評価値")
    )
    detail_lines = [
        ...
        (
            ...
            if is_jra
            else f"NAR正式順位：{rank_display(current_rank)} {nar_top5_mark_from_row(row) or '—'}"
        ),
    ]
    if is_jra:
        detail_lines.extend(
        ...
        attention_reason = nar_short_attention_reason(row) if truthy_display(row.get("nar_warning_candidate")) else ""
        detail_lines.extend(
            [
                f"相手信頼度：{clean_text(pick(row, 'partner_trust_level')) or '—'}",
                f"総合注目度：{nar_total_attention_rank(row)}",
                f"能力首位差：{format_number(pick(row, 'nar_ability_gap_from_top')) or '—'}",
                f"注目理由：{attention_reason or '—'}",
            ]
        )
    ...
    if is_nar:
        main_parts = [
            ...
            odds,
            age,
            f"純能力{ability_value}（{rank_display(ability_rank)}）",
        ]
    else:
        main_parts = [
            mark,
...
def render_market_audit_details(result: PredictionResult, table: pd.DataFrame) -> None:
    with st.expander("詳細/監査情報", expanded=False):
        ...
        if result_race_mode(result) == "nar":
            nar_audit_columns = existing_columns(
                table,
                [
                    "馬番",
                    "馬名",
            ...
            if nar_audit_columns:
                st.write("NAR Top5・購入判定 監査")
                st.dataframe(table.loc[:, nar_audit_columns], use_container_width=True, hide_index=True)
...
def render_race_flow(result: PredictionResult) -> None:
    ...
    body.append("</div>")
    flow_review = race_flow_review_lines(rows, pace, result.race_mode)
    if flow_review:
...
def horse_summary_card_html(
    ...
    group = "" if is_top5_mode else display_group_from_row(row)
    ...
    if is_jra:
        ...
        if mark == "✓":
            reason = nar_short_attention_reason(row)
            if reason:
                quick_items.append(f"✓注目：{reason}")
    ...
    detail_lines = [
        "出走馬詳細",
        (f"{mark_part} {no} {name}".strip() if is_top5_mode else f"【{group}】{mark_part} {no} {name}".strip()),
        f"{age}",
        weight,
        jockey,
        f"騎手成績：{jockey_stats or '—'}",
    ...
    if is_jra:
        ...
        attention_reason = nar_short_attention_reason(row) if mark == "✓" else ""
        detail_lines.extend(
            [
                "",
                ...
                f"総合注目度：{rank_display(card_pick(row, index_row, 'baseline_ver3_current_evaluation_rank', 'ver3_current_evaluation_rank', 'current_evaluation_rank'))}",
    ...
    title = (f"{mark_part} {no} {name}　{odds}" if is_top5_mode else f"【{group}】{mark_part} {no} {name}　{odds}").strip()
    ...
    return (
        ...
        + ("" if is_top5_mode else f'<span class="ka-chip {group.lower()}">{plain_text_to_html(group)}</span>')
...
def nar_warning_reason_display(value: Any) -> str:
    text = clean_text(value)
    if not text:
        return ""
    replacements = {
        "Ver3今回評価Top5": "総合注目度上位",
        "能力外警戒": "能力順位以上に警戒",
        "VER3_ONLY": "総合注目度上位",
        "PURE_ONLY": "純能力Top5のみ",
        "IN/OUT": "",
...
def nar_total_attention_rank(row: dict[str, Any]) -> str:
    return rank_display(pick(row, "baseline_ver3_current_evaluation_rank", "ver3_current_evaluation_rank", "current_evaluation_rank"))


def nar_condition_materials_text(row: dict[str, Any]) -> str:
    parts: list[str] = []
    if clean_text(pick(row, "same_course")) == "★":
        parts.append("★同コース")
    if clean_text(pick(row, "same_distance")) == "★":
        parts.append("★同距離")
    distance_index = to_float(pick(row, "distance_index", "距離指数"))
    course_index = to_float(pick(row, "course_index", "コース指数"))
    if distance_index is not None and distance_index >= 80:
        parts.append(f"距離指数{format_number(distance_index)}")
    if course_index is not None and course_index >= 80:
        parts.append(f"コース指数{format_number(course_index)}")
    return " / ".join(parts)


def nar_short_attention_reason(row: dict[str, Any]) -> str:
    reason = nar_warning_reason_display(row.get("nar_warning_reason"))
    if reason:
        return reason
    if truthy_display(row.get("nar_warning_candidate")):
        return "能力順位以上に警戒"
...
def race_flow_review_lines(rows: list[dict[str, Any]], pace: str, race_mode: str = "") -> list[str]:
    ...
    if clean_text(race_mode).lower() == "nar":
        return nar_race_flow_review_lines(rows, pace)
    front, middle, _back, _unknown = running_position_groups(rows)
    lines = [
...
def nar_race_flow_review_lines(rows: list[dict[str, Any]], pace: str) -> list[str]:
    ...
    top3_numbers = [clean_text(row.get("number")) or clean_text(pick(row, "馬番", "馬")) for row in top5[:3]]
    lines = [
        f"【スタート】\n想定ペースは{pace}。NAR Top5候補の脚質と序盤位置を分けて確認します。",
        f"【4角】\nTop5内で前方想定は{horse_refs(rows, front_top5)}。中団は{horse_refs(rows, middle_top5)}、後方は{horse_refs(rows, back_top5)}です。",
        f"【直線】\n中心候補はTop5 1〜3位の{horse_refs(rows, top3_numbers)}。展開○は{horse_refs(rows, pace_positive)}です。",
    ]
    if warnings:
        lines.append(
            "【注意馬】\n"
            f"Top5外では{horse_refs(rows, warning_numbers)}が注意対象です。"
        )
    return lines
...
def nar_row_position_group(row: dict[str, Any]) -> str:
    group = clean_text(row.get("corner4_group"))
    if group in {"front", "middle", "back"}:
        return group
    label = clean_text(pick(row, "corner4_display", "4角位置", "4角予想", "_estimated_position_corner4_label", "estimated_position_label", "想定位置"))
...
def build_detail_analysis_table(
    ...
    for row in rows:
        ...
        if clean_text(race_mode).lower() == "jra":
            ...
            warning_label = "✓" if truthy_display(row.get("nar_warning_candidate")) and not nar_top5_mark_from_row(row) else ""
            attention_reason = nar_short_attention_reason(row) if warning_label else ""
            record = {
                ...
                "相手信頼度": clean_text(pick(row, "partner_trust_level")) or "—",
                "総合注目度": nar_total_attention_rank(row),
                "✓注目馬": warning_label or "—",
                "注目理由": attention_reason or "—",
                "馬": join_nonempty([no, name], sep=" "),
                "単勝オッズ": format_odds(pick(row, "単勝オッズ", "オッズ", "単勝")) or "—",
                "年齢": clean_text(pick(row, "馬年齢", "性齢", "馬齢")) or "—",
                "騎手": compact_table_jockey_text(row),
                "脚質": short_running_style(row),
