...
def _strong_partners():
    return [
        _horse(1, 1, 100, market_rank=1, odds=2.8, corner4_group="front", same_distance="★", same_course="★"),
...
def _raw_popularity_rows(*, honmei_popularity: int = 1, honmei_odds: float = 2.8, second_score: float = 88):
    return [
        {"馬番": 1, "馬名": "一位", "market_ability_score": 100, "market_ability_rank": 1, "popularity": honmei_popularity, "odds": honmei_odds, "condition_fit_mark": "★", "_estimated_position_corner4_label": "先団"},
        {
            "馬番": 2,
            "馬名": "二位",
...
def test_condition_support_drives_purchase_judgement_to_a():
    result = build_full_field_comparison(_raw_popularity_rows(), race_mode="nar", sort_mode="current")
...
def test_condition_support_reaches_partner_trust_without_market_bonus():
    result = build_full_field_comparison(_raw_popularity_rows(), race_mode="nar", sort_mode="current")
    by_number = {row["number"]: row for row in result["rows"]}

    assert by_number["2"]["partner_trust_level"] == "HIGH"
    assert by_number["3"]["partner_trust_level"] == "HIGH"
    assert "市場上位" not in by_number["2"]["partner_trust_reason"]
    assert by_number["2"]["partner_trust_score"] >= 5.0


def test_purchase_judgement_varies_with_condition_support():
    scenarios = [
...
def test_strong_supported_honmei_is_a_without_automatic_win_bet():
...
def test_odds_under_two_does_not_change_axis_ticket_candidate():
    horses = _strong_partners()
    horses[0]["odds"] = 1.5
    result = annotate_nar_purchase_judgement(horses)
    ...
    assert "取得時オッズ" in result["win_bet_block_reason"]
    assert result["recommended_ticket_mode"] == "AXIS_QUINELLA"
...
def test_market_rank_four_does_not_override_supported_axis():
    horses = _strong_partners()
    ...
    assert result["race_purchase_judgement"] == "A"
    assert result["recommended_ticket_mode"] == "AXIS_QUINELLA"
    assert "◎市場4位" not in result["race_purchase_reason"]
...
def test_nar_comparison_uses_pure_rank_marks_and_keeps_jra_rank_unchanged():
    nar_rows = [
        {"market_ability_rank": 1, "馬番": 1, "馬名": "一位", "market_ability_score": 100, "current_evaluation_rank": 3, "_最終印点": 80, "人気": 1, "単勝オッズ": 2.8},
        {"market_ability_rank": 2, "馬番": 2, "馬名": "二位", "market_ability_score": 90, "current_evaluation_rank": 1, "_最終印点": 100},
        {"market_ability_rank": 3, "馬番": 3, "馬名": "三位", "market_ability_score": 80, "current_evaluation_rank": 2, "_最終印点": 90},
        {"market_ability_rank": 4, "馬番": 4, "馬名": "四位", "market_ability_score": 70, "current_evaluation_rank": 6, "_最終印点": 60},
        {"market_ability_rank": 5, "馬番": 5, "馬名": "五位", "market_ability_score": 60, "current_evaluation_rank": 5, "_最終印点": 70},
        {"market_ability_rank": 6, "馬番": 6, "馬名": "六位", "market_ability_score": 50, "current_evaluation_rank": 4, "_最終印点": 75},
    ]
    ...
    jra_rows = [
        {"market_ability_rank": 6, "馬番": 1, "馬名": "JRA一", "market_ability_score": 50, "v1_final_rank": 2, "jra_top5_score": 50},
        {"market_ability_rank": 7, "馬番": 2, "馬名": "JRA二", "market_ability_score": 40, "v1_final_rank": 1, "jra_top5_score": 60},
    ]
