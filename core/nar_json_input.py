...
class NarJsonDataError(ValueError):
    """Raised when uploaded shortcut JSON cannot be used for NAR prediction."""
...
def build_nar_prediction_inputs_from_uploads(
    ...
    classified = classify_uploaded_json_files(uploaded_files)
    validate_nar_json_bundle(classified)
...
def classify_uploaded_json_files(uploaded_files: Iterable[tuple[str, bytes]]) -> dict[str, dict[str, Any]]:
    ...
    for file_name, raw in uploaded_files:
        try:
            data = load_uploaded_json(raw)
        except NarJsonDataError as exc:
            invalid_files.append(f"{file_name}: {exc}")
            continue
    ...
    if invalid_files:
        raise NarJsonDataError("読み込めないJSONがあります。\n" + "\n".join(invalid_files))
    if duplicates:
        details = [f"{data_type}: {', '.join(names)}" for data_type, names in duplicates.items()]
        raise NarJsonDataError("同じdata_typeのJSONが複数あります。\n" + "\n".join(details))
    return classified
...
def load_uploaded_json(raw: bytes) -> dict[str, Any]:
    ...
    try:
        ...
        if not match:
            raise NarJsonDataError("JSON本文を取得できませんでした。") from None
        try:
            data = json.loads(match.group(0))
        except json.JSONDecodeError as exc:
            raise NarJsonDataError(f"JSON解析に失敗しました: {exc}") from exc
    if not isinstance(data, dict):
        raise NarJsonDataError("JSONの最上位がオブジェクトではありません。")
    return data
...
def validate_nar_json_bundle(classified: dict[str, dict[str, Any]]) -> None:
    ...
    if missing:
        ...
        raise NarJsonDataError(f"必要なJSONが不足しています: {labels}")
    ...
    if not isinstance(entry_horses, list) or not entry_horses:
        raise NarJsonDataError("出馬表JSONのhorsesが空です。")
    ...
    if len(entry_horses) != len(speed_horses):
        raise NarJsonDataError(
            f"出馬表とタイム指数の頭数が一致しません（出馬表{len(entry_horses)}頭 / タイム指数{len(speed_horses)}頭）。"
        )

    entry_numbers = {_horse_number(horse) for horse in entry_horses}
    speed_numbers = {_horse_number(horse) for horse in speed_horses}
    if "" in entry_numbers or "" in speed_numbers:
        raise NarJsonDataError("horse_numberが空の馬があります。")
    if entry_numbers != speed_numbers:
        missing_speed = sorted(entry_numbers - speed_numbers)
        missing_entry = sorted(speed_numbers - entry_numbers)
        detail = []
        if missing_speed:
            detail.append("タイム指数なし: " + ", ".join(missing_speed))
        if missing_entry:
            detail.append("出馬表なし: " + ", ".join(missing_entry))
        raise NarJsonDataError("出馬表とタイム指数の馬番が一致しません。\n" + "\n".join(detail))

    if not isinstance(running_styles, list) or not running_styles:
        raise NarJsonDataError("コース傾向JSONのrunning_stylesが空です。")
...
def _suggested_name(data: dict[str, Any], race_id: str, fallback_type: str) -> str:
    return str(data.get("suggested_file_name") or f"{race_id}_{fallback_type}.html")
